...
{
    /// <summary>
    /// This class is an auto=generated stub for <see cref="Temporal.Prototypes.SampleApp42.SampleAvWorkflowImplementation" />
    /// (in assembly "ClientApp, Version=1.0.0.0, Culture=neutral, PublicKeyToken=null").
    /// </summary>
    internal partial class MockWorkflowStub
    {
        private readonly IWorkflowHandle _workflowHandle;

        public MockWorkflowStub(IWorkflowHandle workflowHandle)
        {
        ...
        public Task<Temporal.Prototypes.SampleApp42.SampleAvWfResult> ExecWorkflowAsync(Temporal.Prototypes.SampleApp42.SampleAvWfInput input, CancellationToken cancelToken = default)
        {
            return ExecWorkflowAsync(input, signalConfig: null, cancelToken);
        }

        public async Task<Temporal.Prototypes.SampleApp42.SampleAvWfResult> ExecWorkflowAsync(Temporal.Prototypes.SampleApp42.SampleAvWfInput input, SignalWorkflowConfiguration signalConfig, CancellationToken cancelToken = default)
        {
            const string WorkflowTypeName = "ExecWorkflow";

            Console.WriteLine($"ExecWorkflowAsync(..) was invoked to execute the workflow via {this.GetType().Name}:"
                            + $" WorkflowTypeName=\"{WorkflowTypeName}\".");

            await Task.Delay(millisecondsDelay: 1);
            return default(Temporal.Prototypes.SampleApp42.SampleAvWfResult);
        }
